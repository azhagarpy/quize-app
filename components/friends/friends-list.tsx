...
import { UserPlus, Search, UserCheck, UserX, Mail } from "lucide-react"
...
export function FriendsList({ userId, roomId }: { userId: string; roomId?: string }) {
  ...
  const inviteFriendToRoom = async (friendId: string, friendUsername: string) => {
    ...
    try {
      // Create a notification in the database (you would need to create this table)
    ...
    <Card className="game-card">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          <span>Friends</span>
          {pendingRequests.length > 0 && <Badge className="ml-auto bg-primary">{pendingRequests.length}</Badge>}
        </CardTitle>
      </CardHeader>
      <CardContent>
